...
NOTE: CORS preflight requests that are handled before application route matching do not create normal server HTTP observations.
This avoids creating observations or exported metrics with high-cardinality URI values when no route template is available for the `OPTIONS` request.
