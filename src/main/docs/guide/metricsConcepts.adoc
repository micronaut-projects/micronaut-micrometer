...
To add request-aware custom tags to the built-in HTTP server and client metrics, create a bean that implements `HttpMetricsTagProvider`.
Each provider is invoked when Micronaut records `http.server.requests` or `http.client.requests`.

.Example
[source,java]
----
include::{testsmetricscore}/HttpMetricsTagProviderExample.java[tags=imports;class]
----
