...
.Example
snippet::io.micronaut.docs.metrics.configurer.SimpleMeterRegistryConfigurer[tags="imports,class"]
...
.Example
snippet::io.micronaut.docs.metrics.filter.MeterFilterFactory[tags="imports,class"]
...
NOTE: CORS preflight requests that are handled before application route matching are not recorded as normal server request metrics.
This avoids creating `http.server.requests` metrics with high-cardinality URI tags when no route template is available for the `OPTIONS` request.
...
dependency:hibernate-micrometer[groupId="org.hibernate"]
...
Currently, the following binders are provided to instrument Netty event loops and server channels:

* *Executor binder*: When `micronaut.metrics.binders.executor.enabled` is enabled, configured Netty event loop groups also expose `executor.queued` and `executor.pool.size` metrics tagged with the event loop group name.
* *Netty queue binder*: Instrument and expose Netty event loop queue metrics for event loop groups used by the server and HTTP clients; use `micronaut.metrics.binders.netty.queues.enabled` to toggle. Default is *false*. The queues' size and tasks wait and execution time are exposed.
...
snippet::io.micronaut.docs.metrics.custom.IndexController[tags="imports,class"]

[.lang-python]
NOTE: A Python controller with constrained parameters (`NotBlank`) needs an explicit `@Validated` on the class, and a reactive method returns a `Publisher` annotated with `@SingleResult`.
...
snippet::io.micronaut.docs.metrics.exception.OutOfStockException[tags="imports,class"]
