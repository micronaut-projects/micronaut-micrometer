...
If you are using `io.micronaut.sql:micronaut-jdbc-hikari` you will get additional pool metrics as HikariCP has built in support for meter registries.
...
If you are using `io.micronaut.sql:micronaut-jdbc-tomcat` or `io.micronaut.sql:micronaut-jdbc-dbcp` you will get the following metrics
