...
If `projectId` is not configured explicitly, Micronaut falls back to Google Cloud's default project ID discovery.

|=======
|*Name* |*Description*
|enabled |Whether to enable the reporter, for example per-environment or for local development. Default: `true`
|projectId |Stackdriver Project ID. If omitted, Micronaut falls back to Google Cloud default project ID discovery.
