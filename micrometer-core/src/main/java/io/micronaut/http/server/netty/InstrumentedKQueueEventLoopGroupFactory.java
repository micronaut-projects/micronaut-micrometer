...
public class InstrumentedKQueueEventLoopGroupFactory implements EventLoopGroupFactory {
    ...
    public EventLoopGroup createEventLoopGroup(int threads, @Nullable Integer ioRatio) {
        return new KQueueEventLoopGroup(threads, (Executor) null,
    ...
    public EventLoopGroup createEventLoopGroup(int threads, ThreadFactory threadFactory, @Nullable Integer ioRatio) {
        return new KQueueEventLoopGroup(threads, (Executor) threadFactory == null ? null : new ThreadPerTaskExecutor(threadFactory),
                DefaultEventExecutorChooserFactory.INSTANCE,
                DefaultSelectStrategyFactory.INSTANCE,
                RejectedExecutionHandlers.reject(),
                instrumentedEventLoopTaskQueueFactory);
    ...
    public EventLoopGroup createEventLoopGroup(@Nullable Integer ioRatio) {
        return new KQueueEventLoopGroup(0, (Executor) null,
                DefaultEventExecutorChooserFactory.INSTANCE,
                DefaultSelectStrategyFactory.INSTANCE,
                RejectedExecutionHandlers.reject(),
                instrumentedEventLoopTaskQueueFactory);
